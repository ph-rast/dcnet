...
#include /functions/cov2cor.stan
  // Multithreading with reduced sums
  ...
  real partial_log_lik_lpmf(array[] int seq, int start, int end,
			    data int nt, data array[] matrix rts,
			    array[] matrix H, array[] vector mu){
    real ptarget = 0;
    // What does multi_normal_lpdf wirte out? real or vector?
    ptarget += multi_normal_lpdf(rts[1,1:2] | mu[1,1], H[1,1])
    return ptarget;
  }
}
