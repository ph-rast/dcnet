...
#include /model_components/mu.stan
      ...
      c_h[j] = c_h_fixed + c_h_random[j]; // enters as exp(c_h) in model
      //c_h[j] = c_h_fixed;
      ...
      a_h_sum[j] = rep_vector(1.0, nt) ./ (1 + exp(-(a_h_fixed)) );
      ...
      b_h_sum[j] = rep_vector(1.0, nt) ./ (1 + exp(-(b_h_fixed)) );
	...
	D[j, t, d] = sqrt( vd[j,d] );
      ...
      Qr[j,t ] = (1 - a_q - b_q) * S + a_q * (u[j, t-1 ] * u[j, t-1 ]') + b_q * Qr[j, t-1];// S and UU' define dimension of Qr
...
model {
  ...
  to_vector(c_h_fixed) ~ normal(0,3);
