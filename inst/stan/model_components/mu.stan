...
phi[j] = vec_phi_fixed +
  (diag_pre_multiply(phi_tau, diag_matrix(rep_vector(1.0, nt)))*phi_stdnorm[j]) ;


mu[j,t] = phi0[j] + to_matrix( phi[j], nt, nt ) * rts[j, t-1]';//(rts[j,t-1]'- phi0_fixed) ; //to_matrix fill by column major order
