...
transformed parameters {
  // copy data to transformed parameter to force it into reported parameters for printing later
  vector[Sdim] S_vec_tau = S_vec_tau_fixed;
...
model {
  ...
  for( j in 1:J) {  
    ...
    if ( distribution == 0 ) {
      for(t in 1:T){
	rts[j,t] ~ multi_normal(mu[j,t], multiply_lower_tri_self_transpose(L_H[j,t]));
      }
    } else if ( distribution == 1 ) {
      for(t in 1:T){
	rts[j,t] ~ multi_student_t(nu, mu[j,t], multiply_lower_tri_self_transpose(L_H[j,t]));
      }
    }
  }
}
