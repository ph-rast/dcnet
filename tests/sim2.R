...
ns  <- c(50, 100)#c(25, 50, 100)
tls <- c(50, 100)#c(25, 50, 100)
simcond <- expand.grid(N = ns, tl = tls)
n_conds <- nrow(simcond)
replications <- 10

# register one worker per condition:
cl <- makeCluster(min(n_conds, 56))
...
#########################
# 1) set number of reps to 100
n_reps     <- 10

# 2) rebuild your task grid: 9 conditions × 100 reps = 900 tasks
task_grid <- expand.grid(
  idx = seq_len(n_conds), 
  rep = seq_len(n_reps)
)

# 3) launch your cluster (still 56 workers)
cl <- makeCluster(56)
registerDoParallel(cl)

# 4) foreach over all 900 tasks, with dynamic scheduling 
#    (so workers grab new tasks as they finish):
all_results <- foreach(
  t = seq_len(nrow(task_grid)),
  .combine   = rbind,
  .packages  = c("dcnet","loo","posterior"),
  ...
  fit <- safe_sample(s, replication_data = dat)
  if (is.null(fit)) return(NULL)

  do.call(rbind, lapply(seq_along(variables_m), function(p) {
    stan_par <- variables_m[p]
    sim_var  <- var[p]

    draws_mat <- fit$model_fit$draws(variables = stan_par, format = "matrix")
    truth_vec <- unlist(dat[[3]][[sim_var]])

    SF <- if (stan_par == "S_vec_tau") {
      tmp <- data.frame(t(apply(fit$S_vec_tau_post, 2,
                          function(x) quantile(x, c(0.5,0.025,0.975)))))
      colnames(tmp) <- c("mean","q2.5","q97.5")
      tmp
    } else {
      fit$model_fit$summary(
        variables = stan_par, "mean",
        extra_quantiles = ~ posterior::quantile2(., probs = c(.025, .975))
      )
    }

    cov   <- mean(truth_vec > SF$q2.5 & truth_vec < SF$q97.5)
    all_d <- as.numeric(draws_mat)
    all_t <- rep(truth_vec, each = nrow(draws_mat))
    bias  <- mean(all_d - all_t)
    rmse  <- sqrt(mean((all_d - all_t)^2))
    ciw   <- mean(abs(SF$q97.5 - SF$q2.5))
    looic <- looic(fit)
...
# 5) Aggregate across 100 reps for each (N, tl, parameter)
library(dplyr)
